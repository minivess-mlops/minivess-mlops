...
|----|----------|-----------------|------------|----------------|---------------------|---------------|--------------|
| H1 | **Multi-stage Dockerfile + uv cache mounts** --- Further optimize existing multi-stage build: strip `__pycache__`, `.pyc` files, test dependencies; use `--no-compile` in runner; audit `.venv` for unused extras | 40-60% size reduction (9 GB to 3.5-5.5 GB) | LOW (2 hr) | Yes | Yes | Dockerfile edit only | [Astral uv Docker guide](https://docs.astral.sh/uv/guides/integration/docker/) |
