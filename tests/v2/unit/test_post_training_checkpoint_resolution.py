...
in priority order: best.ckpt → best_val_loss.pth → last.pth → epoch_*.ckpt.
"""
...
class TestCheckpointFallbackChain:
    """Checkpoint resolution must follow priority: best.ckpt > best_val_loss.pth > last.pth > epoch_*."""

    def test_prefers_best_ckpt(self, tmp_path: Path) -> None:
        """best.ckpt is preferred over all alternatives."""
        fold_dir = tmp_path / "fold_0"
        fold_dir.mkdir()
        _write_dummy_ckpt(fold_dir / "best.ckpt")
    ...
    def test_fallback_to_best_val_loss(self, tmp_path: Path) -> None:
        """Falls back to best_val_loss.pth when best.ckpt absent."""
        fold_dir = tmp_path / "fold_0"
        fold_dir.mkdir()
        _write_dummy_ckpt(fold_dir / "best_val_loss.pth")
        _write_dummy_ckpt(fold_dir / "last.pth")
    ...
    def test_fallback_to_last(self, tmp_path: Path) -> None:
        """Falls back to last.pth when best variants absent."""
        fold_dir = tmp_path / "fold_0"
        fold_dir.mkdir()
        _write_dummy_ckpt(fold_dir / "last.pth")
        _write_dummy_ckpt(fold_dir / "epoch_001.ckpt")
...
class TestMultiFoldDiscovery:
    ...
    def test_skips_missing_fold_dirs(self, tmp_path: Path) -> None:
        ...
        fold_0.mkdir()
        _write_dummy_ckpt(fold_0 / "best.ckpt")
