...
- If FORCE_STOP -> Output escape hatch report (below), then [task-selection.md](task-selection.md) (skip to next task)
- If plan converged -> Output final summary and exit
- If session budget exceeded -> Output session summary and suggest new session

## Escape Hatch Report (FORCE_STOP)

When a task hits FORCE_STOP, output a structured report before moving on. This prevents knowledge loss and helps the next session or human reviewer:

```
FORCE_STOP REPORT — Task {task_id}
  Iterations used: {N}/{max}
  Residual failures: {count}
  What was attempted:
    1. {fix attempt 1}
    2. {fix attempt 2}
    ...
  Root cause hypothesis: {best guess at why it's stuck}
  Suggested next steps:
    - {specific action 1}
    - {specific action 2}
  Append to LEARNINGS.md: YES
```
