...
## Purpose
This PRD is written for an **academic software engineering project**. All references
must follow academic citation standards. This guide is the canonical reference for
citation formatting across all PRD files.

## Citation Architecture

### Central Bibliography
`docs/planning/prd/bibliography.yaml` is the single source of truth for all cited works.
...
- If key word needed for clarity: add it after year (e.g., `hatamizadeh2022swinunetr`)
...
the author-year citation. Example:

```yaml
rationale: >
  SegResNet (Myronenko, 2019) serves as baseline (0.25 prior) due to
  its proven performance and lightweight architecture. VISTA-3D
  (He et al., 2024) matches at 0.25 given its foundation model
  capabilities for interactive 3D segmentation. The nnU-Net framework
  (Isensee et al., 2021) remains the benchmark standard at medical
...
the PRD decision network. These are **sub-citations** — papers cited BY the
ingested paper that provide evidence for other decision nodes.

### Process
1. Read the ingested paper's reference list
2. Identify references relevant to PRD decision nodes
3. Add each sub-citation to `bibliography.yaml`
4. Link sub-citations to the appropriate `.decision.yaml` files
5. Note in the parent paper's entry which sub-citations were extracted

### Example
```
Ingesting: He et al. (2024) "VISTA3D"
Sub-citations extracted:
  - Myronenko (2019) → segmentation_models (SegResNet baseline)
  - Hu et al. (2022) → foundation_model_integration (LoRA fine-tuning)
